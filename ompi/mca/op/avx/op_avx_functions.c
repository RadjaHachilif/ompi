...
#define OP_AVX_AVX512_FUNC(name, type_sign, type_size, type, op)	       \
    if( OMPI_OP_AVX_HAS_AVX512_FLAG & mca_op_avx_component.flags ) {           \
...
#define OP_AVX_AVX2_FUNC(name, type_sign, type_size, type, op)	               \
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {             \
...
#define OP_AVX_AVX_FUNC(name, type_sign, type_size, type, op)	               \
    if( OMPI_OP_AVX_HAS_AVX_FLAG & mca_op_avx_component.flags ) {              \
...
{                                                                              \
    ...
    OP_AVX_AVX_FUNC(name, type_sign, type_size, type, op);		       \
    while( left_over > 0 ) {                                                   \
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_MUL(name, type_sign, type_size, type, op)		\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {             \
...
{                                                                       \
    ...
    OP_AVX_AVX2_MUL(name, type_sign, type_size, type, op);		\
    while( left_over > 0 ) {						\
        int how_much = (left_over > 8) ? 8 : left_over;			\
	switch(how_much) {						\
        case 8: out[7] = current_func(out[7], in[7]);			\
...
#if defined(OMPI_MCA_OP_HAVE_AVX) && (1 == OMPI_MCA_OP_HAVE_AVX)
#define OP_AVX_AVX_BIT_FUNC(name, type_size, type, op)			\
    if( OMPI_OP_AVX_HAS_AVX_FLAG & mca_op_avx_component.flags ) {              \
        types_per_step = (128 / 8) / sizeof(type);			\
	for( ; left_over >= types_per_step; left_over -= types_per_step ) { \
	    __m128i vecA = _mm_lddqu_si128((__m128i*)in);			\
...
{									\
    ...
    OP_AVX_AVX_BIT_FUNC(name, type_size, type, op);			\
    while( left_over > 0 ) {						\
        int how_much = (left_over > 8) ? 8 : left_over;			\
	switch(how_much) {						\
        case 8: out[7] = current_func(out[7], in[7]);			\
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_FLOAT_FUNC(op)					\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {      \
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_DOUBLE_FUNC(op)					\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {      \
...
#if defined(OMPI_MCA_OP_HAVE_AVX) && (1 == OMPI_MCA_OP_HAVE_AVX)
#define OP_AVX_AVX_FUNC_3(name, type_sign, type_size, type, op)		\
    if( OMPI_OP_AVX_HAS_AVX_FLAG & mca_op_avx_component.flags ) {	\
        types_per_step = (128 / 8) / sizeof(type);			\
	for( ; left_over >= types_per_step; left_over -= types_per_step ) { \
...
{                                                                       \
    ...
    OP_AVX_AVX_FUNC_3(name, type_sign, type_size, type, op);		\
    while( left_over > 0 ) {						\
        int how_much = (left_over > 8) ? 8 : left_over;			\
	switch(how_much) {						\
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_MUL_3(name, type_sign, type_size, type, op)	\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {	\
        types_per_step = (128 / 8) / sizeof(type);			\
	for (; left_over >= types_per_step; left_over -= types_per_step) { \
...
{                                                                       \
    ...
    OP_AVX_AVX2_MUL_3(name, type_sign, type_size, type, op);		\
    while( left_over > 0 ) {						\
        int how_much = (left_over > 8) ? 8 : left_over;			\
	switch(how_much) {						\
        case 8: out[7] = current_func(in1[7], in2[7]);			\
...
#if defined(OMPI_MCA_OP_HAVE_AVX) && (1 == OMPI_MCA_OP_HAVE_AVX)
#define OP_AVX_AVX_BIT_FUNC_3(name, type_size, type, op)		\
    if( OMPI_OP_AVX_HAS_AVX_FLAG & mca_op_avx_component.flags ) {	\
        types_per_step = (128 / 8) / sizeof(type);			\
        for( ; left_over >= types_per_step; left_over -= types_per_step ) {	\
	    __m128i vecA = _mm_lddqu_si128((__m128i*)in1);		\
...
{                                                                       \
    ...
    OP_AVX_AVX_BIT_FUNC_3(name, type_size, type, op);			\
    while( left_over > 0 ) {						\
        int how_much = (left_over > 8) ? 8 : left_over;			\
	switch(how_much) {						\
        case 8: out[7] = current_func(in1[7], in2[7]);			\
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_FLOAT_FUNC_3(op)					\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {      \
        types_per_step = (256 / 8) / sizeof(float);			\
	for( ; left_over >= types_per_step; left_over -= types_per_step ) { \
...
#if defined(OMPI_MCA_OP_HAVE_AVX2) && (1 == OMPI_MCA_OP_HAVE_AVX2)
#define OP_AVX_AVX2_DOUBLE_FUNC_3(op)					\
    if( OMPI_OP_AVX_HAS_AVX2_FLAG & mca_op_avx_component.flags ) {	\
        types_per_step = (256 / 8) / sizeof(double);			\
	for( ; left_over >= types_per_step; left_over -= types_per_step ) { \
